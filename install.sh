...
cleanup_pnpm_openclaw_paths() {
    local pnpm_root=""
    pnpm_root="$(pnpm root -g 2>/dev/null || true)"
    if [[ -z "$pnpm_root" || "$pnpm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$pnpm_root"/.openclaw-* "$pnpm_root"/openclaw 2>/dev/null || true
}
...
cleanup_openclaw_bin_conflict() {
    ...
    if [[ -n "$pnpm_bin" && "$bin_path" != "$pnpm_bin/openclaw" ]]; then
        case "$bin_path" in
...
pnpm_log_indicates_missing_build_tools() {
    local log="$1"
    if [[ -z "$log" || ! -f "$log" ]]; then
        return 1
    fi
...
auto_install_build_tools_for_pnpm_failure() {
    local log="$1"
    if ! pnpm_log_indicates_missing_build_tools "$log"; then
        return 1
    fi
...
run_pnpm_global_install() {
    local spec="$1"
    local log="$2"

    local -a cmd
    cmd=(env "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" pnpm --loglevel "$PNPM_LOGLEVEL")
    if [[ -n "$PNPM_SILENT_FLAG" ]]; then
        cmd+=("$PNPM_SILENT_FLAG")
    fi
    cmd+=(--no-fund --no-audit add -g "$spec")
    local cmd_display=""
...
extract_first_npm_error_line() {
    local log="$1"
    grep -E '(npm|pnpm) (ERR!|error)|ERR!' "$log" | head -n1 || true
}

extract_npm_error_code() {
    local log="$1"
    sed -n -E 's/^(npm|pnpm) (ERR!|error) code[[:space:]]+([^[:space:]]+).*$/\3/p' "$log" | head -n1
}

extract_npm_error_syscall() {
    local log="$1"
    sed -n -E 's/^(npm|pnpm) (ERR!|error) syscall[[:space:]]+(.+)$/\3/p' "$log" | head -n1
}

extract_npm_error_errno() {
    local log="$1"
    sed -n -E 's/^(npm|pnpm) (ERR!|error) errno[[:space:]]+(.+)$/\3/p' "$log" | head -n1
}

print_pnpm_failure_diagnostics() {
    local spec="$1"
    local log="$2"
    local debug_log=""
    local first_error=""
...
install_openclaw_pnpm() {
    ...
    local log
    log="$(mktempfile)"
    if ! run_pnpm_global_install "$spec" "$log"; then
        local attempted_build_tool_fix=false
        if auto_install_build_tools_for_pnpm_failure "$log"; then
            attempted_build_tool_fix=true
            ui_info "Retrying pnpm install after build tools setup"
            if run_pnpm_global_install "$spec" "$log"; then
                ui_success "OpenClaw pnpm package installed"
                return 0
            fi
        fi

        print_pnpm_failure_diagnostics "$spec" "$log"

        if [[ "$VERBOSE" != "1" ]]; then
            if [[ "$attempted_build_tool_fix" == "true" ]]; then
                ui_warn "pnpm install still failed after build tools setup; showing last log lines"
            else
                ui_warn "pnpm install failed; showing last log lines"
            fi
            tail -n 80 "$log" >&2 || true
        fi

        if grep -q "ENOTEMPTY: directory not empty, rename .*openclaw" "$log"; then
            ui_warn "pnpm left stale directory; cleaning and retrying"
            cleanup_pnpm_openclaw_paths
            if run_pnpm_global_install "$spec" "$log"; then
                ui_success "OpenClaw pnpm package installed"
                return 0
        ...
        if grep -q "EEXIST" "$log"; then
            ...
            ui_error "pnpm failed because an openclaw binary already exists"
            if [[ -n "$conflict" ]]; then
                ui_info "Remove or move ${conflict}, then retry"
            fi
            ui_info "Or rerun with: pnpm add -g --force ${spec}"
        fi
        return 1
    fi
    ui_success "OpenClaw pnpm package installed"
...
map_legacy_env "OPENCLAW_PNPM_LOGLEVEL" "CLAWDBOT_PNPM_LOGLEVEL"
...
PNPM_LOGLEVEL="${OPENCLAW_PNPM_LOGLEVEL:-error}"
PNPM_SILENT_FLAG="--silent"
...
Options:
  --install-method, --method pnpm|git   Install via pnpm (default) or from a git checkout
  --pnpm                               Shortcut for --install-method pnpm
  --git, --github                     Shortcut for --install-method git
  --version <version|dist-tag>         pnpm install: version (default: latest)
  ...
  --verbose                             Print debug output (set -x, pnpm verbose)
  --help, -h                            Show this help

Environment variables:
  OPENCLAW_INSTALL_METHOD=git|pnpm
  ...
  OPENCLAW_VERBOSE=1
  OPENCLAW_PNPM_LOGLEVEL=error|warn|notice  Default: error (hide pnpm deprecation noise)
...
configure_verbose() {
    ...
    if [[ "$PNPM_LOGLEVEL" == "error" ]]; then
        PNPM_LOGLEVEL="notice"
    fi
    PNPM_SILENT_FLAG=""
    set -x
}
...
Choose install method"
        selection="$("$GUM" choose \
            ...
            "pnpm  · install globally via pnpm" < /dev/tty || true)"
...
Choose install method:
  ...
  2) Install global via pnpm (migrate away from git)
Enter 1 or 2:
EOF
)" || true)"
...
print_active_node_paths() {
    ...
    local node_path node_version pnpm_path pnpm_version
    ...
    if command -v pnpm &> /dev/null; then
        ...
        ui_info "Active pnpm: ${pnpm_version:-unknown} (${pnpm_path:-unknown})"
    fi
    return 0
}
...
find_openclaw_entry_path() {
    ...
    local entry_js="${pnpm_root}/openclaw/dist/entry.js"
    if [[ -f "$entry_js" ]]; then
        echo "$entry_js"
        return 0
    fi
    local entry_mjs="${pnpm_root}/openclaw/dist/entry.mjs"
    if [[ -f "$entry_mjs" ]]; then
        echo "$entry_mjs"
        return 0
    fi
...
# Fix pnpm permissions for global installs (Linux)
fix_pnpm_permissions() {
    ...
    local pnpm_prefix
    pnpm_prefix="$(pnpm config get prefix 2>/dev/null || true)"
    if [[ -z "$pnpm_prefix" ]]; then
        return 0
    fi

    if [[ -w "$pnpm_prefix" || -w "$pnpm_prefix/lib" ]]; then
        return 0
    fi

    ui_info "Configuring pnpm for user-local installs"
    mkdir -p "$HOME/.pnpm-global"
    pnpm config set prefix "$HOME/.pnpm-global"

    # shellcheck disable=SC2016
    local path_line='export PATH="$HOME/.pnpm-global/bin:$PATH"'
    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
        if [[ -f "$rc" ]] && ! grep -q ".pnpm-global" "$rc"; then
            echo "$path_line" >> "$rc"
        fi
    done

    export PATH="$HOME/.pnpm-global/bin:$PATH"
    ui_success "pnpm configured for user installs"
}

ensure_openclaw_bin_link() {
    local pnpm_root=""
    pnpm_root="$(pnpm root -g 2>/dev/null || true)"
    if [[ -z "$pnpm_root" || ! -d "$pnpm_root/openclaw" ]]; then
        return 1
    fi
    local pnpm_bin=""
    ...
    if [[ ! -x "${pnpm_bin}/openclaw" ]]; then
        ln -sf "$pnpm_root/openclaw/dist/entry.js" "${pnpm_bin}/openclaw"
        ui_info "Created openclaw bin link at ${pnpm_bin}/openclaw"
    fi
    return 0
}
...
ensure_pnpm() {
    ...
    corepack enable >/dev/null 2>&1 || true
    if ! run_quiet_step "Activating pnpm" corepack prepare pnpm@10 --activate; then
        ui_warn "Corepack pnpm activation failed; trying manual install"
    fi
    refresh_shell_command_cache
    if detect_pnpm_cmd && pnpm_cmd_is_ready; then
        ui_success "pnpm ready ($(pnpm_cmd_pretty))"
...
EOF
    ...
    ui_info "Install pnpm globally (corepack enable && corepack prepare pnpm@10 --activate) and retry"
    return 1
}
...
pnpm_global_bin_dir() {
    local prefix=""
    prefix="$(pnpm prefix -g 2>/dev/null || true)"
    if [[ -n "$prefix" ]]; then
        if [[ "$prefix" == /* ]]; then
            echo "${prefix%/}/bin"
            return 0
...
warn_openclaw_not_found() {
    ...
    local pnpm_prefix=""
    pnpm_prefix="$(pnpm prefix -g 2>/dev/null || true)"
    local pnpm_bin=""
    pnpm_bin="$(pnpm_global_bin_dir 2>/dev/null || true)"
    if [[ -n "$pnpm_prefix" ]]; then
        echo -e "pnpm prefix -g: ${INFO}${pnpm_prefix}${NC}"
    fi
    if [[ -n "$pnpm_bin" ]]; then
...
resolve_beta_version() {
    local beta=""
    beta="$(pnpm view openclaw dist-tags.beta 2>/dev/null || true)"
...
install_openclaw() {
    ...
    resolved_version="$(pnpm view "${package_name}@${OPENCLAW_VERSION}" version 2>/dev/null || true)"
    ...
    if ! install_openclaw_pnpm "${install_spec}"; then
        ui_warn "pnpm install failed; retrying"
        cleanup_pnpm_openclaw_paths
        install_openclaw_pnpm "${install_spec}"
    fi

    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "openclaw" ]]; then
        if ! resolve_openclaw_bin &> /dev/null; then
            ui_warn "pnpm install openclaw@latest failed; retrying openclaw@next"
            cleanup_pnpm_openclaw_paths
            install_openclaw_pnpm "openclaw@next"
        fi
    fi
...
resolve_openclaw_version() {
    ...
    if [[ -z "$version" ]]; then
        local pnpm_root=""
        pnpm_root=$(pnpm root -g 2>/dev/null || true)
        if [[ -n "$pnpm_root" && -f "$pnpm_root/openclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${pnpm_root}/openclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found OpenClaw checkout but no TTY; defaulting to pnpm install"
            INSTALL_METHOD="pnpm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|pnpm (or set OPENCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" != "pnpm" && "$INSTALL_METHOD" != "git" ]]; then
        ui_error "invalid --install-method: ${INSTALL_METHOD}"
        echo "Use: --install-method pnpm|git"
        exit 2
    fi
    ...
    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up pnpm global install if switching to git
        if pnpm list -g openclaw &>/dev/null; then
            ui_info "Removing pnpm global install (switching to git)"
            pnpm remove -g openclaw 2>/dev/null || true
            ui_success "pnpm global install removed"
        fi
        ...
        # Clean up git wrapper if switching to pnpm
        if [[ -x "$HOME/.local/bin/openclaw" ]]; then
            ui_info "Removing git wrapper (switching to pnpm)"
            rm -f "$HOME/.local/bin/openclaw"
            ui_success "git wrapper removed"
        fi

        # Step 3: Git (required for pnpm installs that may fetch from git or apply patches)
        if ! check_git; then
            install_git
        fi

        # Step 4: pnpm permissions (Linux)
        fix_pnpm_permissions
    ...
    # PATH warning: installs can succeed while the user's login shell still lacks pnpm's global bin dir.
    local pnpm_bin=""
    pnpm_bin="$(pnpm_global_bin_dir || true)"
    if [[ "$INSTALL_METHOD" == "pnpm" ]]; then
        warn_shell_path_missing_dir "$pnpm_bin" "pnpm global bin dir"
    fi
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        if [[ -x "$HOME/.local/bin/openclaw" ]]; then
    ...
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "Reborn from the boiling waters of pnpm. Stronger now."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to pnpm" "curl -fsSL --proto '=https' --tlsv1.2 https://openclaw.ai/install.sh | bash -s -- --install-method pnpm"
